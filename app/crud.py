...
async def get_item_by_qs(session: AsyncSession, title: str | None,
                         description: str | None, price: float | None, author: str | None):
    query = select(models.Adv)
    
    filters = []
    if title:
        filters.append(models.Adv.title == title)
    if description:
        filters.append(models.Adv.description.ilike(f"%{description}%"))
    if price:
        filters.append(models.Adv.price == price)
    if author:
        filters.append(models.Adv.author == author)
